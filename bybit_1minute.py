...
def get_minute_bar(sym):
    ...
    t = (datetime.datetime.now() - datetime.timedelta(hours=3)).strftime('%Y-%m-%d %H:%M:%S')
...
def RSI(sym):
    ...
    df['AU'] = df['상승폭'].ewm(alpha=1/5, min_periods=5).mean()
    df['AD'] = df['하락폭'].ewm(alpha=1/5, min_periods=5).mean()
    ...
    try:
        ...
        if balances()['position_margin'] == 0:
            ...
            if before_rsi < rsi_down and before_rsi < now_rsi:
                ...
                # 매수 한 다음 5분봉 기준 시간
                after_time = get_minute_bar(btc).iloc[[-1]].index[0] + datetime.timedelta(minutes=1)
